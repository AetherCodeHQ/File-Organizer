...
> File-Organizer - Open source tool by AetherCodeHQ

`cli` `file-management` `golang` `organization` `utilities`

## What is it?

**File Organizer** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./File-Organizer --path ./target

# With options
./File-Organizer --path ./target --format json --output report.json

# Verbose mode
./File-Organizer --path ./target --verbose
```
...
### Basic scan
```bash
./File-Organizer --path ./my-project
```

### JSON report
```bash
./File-Organizer --path ./my-project --format json --output report.json
```
...
- name: Run File Organizer
  run: ./File-Organizer --path . --format json --output report.json
```
